...
class InceptionV1(Inception):
    ...
    def get_inception_v1_block(cls, _input, filter_1x1,
        ...
        conv1 = keras.layers.Conv2D(filter_1x1, kernel_size=1, activation='relu',
                                    padding='same', name=name+'_1x1')(_input)

        conv3_reduce = keras.layers.Conv2D(filter_3x3_reduce, kernel_size=1, activation='relu',
                                           padding='same', name=name+'_reduce_1')(_input)
        conv3 = keras.layers.Conv2D(filter_3x3, kernel_size=3, activation='relu',
                                    padding='same', name=name+'_3x3')(conv3_reduce)

        conv5_reduce = keras.layers.Conv2D(filter_5x5_reduce, kernel_size=1, activation='relu',
                                           padding='same', name=name+'_reduce_2')(_input)
        conv5 = keras.layers.Conv2D(filter_5x5, kernel_size=5, activation='relu',
                                    padding='same', name=name+'_5x5')(conv5_reduce)

        maxpool = keras.layers.MaxPool2D((3, 3), strides=(1, 1), padding='same', name=name+'_max_3x3')(_input)
        maxpool = keras.layers.Conv2D(filter_pool, kernel_size=1, activation='relu',
                                      padding='same', name=name+'_reduce_3')(maxpool)
    ...
    def define_model(self):
        ...
        inception1 = self.get_inception_v1_block(stem, 64, 96, 128, 16, 32, 32, 'inception_3a')
        ...
        auxiliary_1 = self.get_auxiliary_output(inception3, 'auxiliary_1')
        ...
        auxiliary_2 = self.get_auxiliary_output(inception6, 'auxiliary_2')
        ...
        output = keras.layers.Dense(self.num_of_classes, activation='softmax', name='output')(fc1)

        self.model = keras.Model(inputs=inputs, outputs=[output, auxiliary_1, auxiliary_2])
